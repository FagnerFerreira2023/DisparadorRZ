  ...
  <div class="WAL position-relative bg-grey-3" :style="style">
    <q-layout class="WAL__layout shadow-3" container view="lHr LpR lFr">
      ...
      <q-toolbar class="q-pr-none q-gutter-xs full-width" style="height: 64px">
          ...
          <q-btn style="margin-right: 5px;" flat class="bg-padrao btn-rounded" icon="mdi-home" @click="() => $router.push({ name: 'home' })" :disable="loadingMount">
        ...
        <!-- Barra de ações em massa moderna -->
        <transition name="slide-down">
          <div v-if="modoSelecaoMassa && ticketsSelecionados.size > 0" class="bulk-actions-bar">
            <div class="bulk-actions-content">
              <div class="bulk-actions-info">
                <q-icon name="mdi-checkbox-multiple-marked" size="24px" class="q-mr-sm" />
                <span class="text-weight-bold">
                  {{ ticketsSelecionados.size }} {{ ticketsSelecionados.size === 1 ? $t('generalSupport.ticketSelected') || 'ticket selecionado' : $t('generalSupport.ticketsSelected') || 'tickets selecionados' }}
                </span>
              </div>
              ...
              <div class="bulk-actions-buttons">
                ...
                >
                  <q-list>
                    <q-item 
                      clickable 
                      v-close-popup 
                      @click="abrirModalTransferirMassa"
                    >
                      <q-item-section avatar>
                        <q-icon name="mdi-transfer" color="primary" />
                      </q-item-section>
                      <q-item-section>
                        <q-item-label>{{ $t('atendimentoInfoCabecalho.transfer') }}</q-item-label>
                      </q-item-section>
                    </q-item>
                    ...
                    <q-item 
                      clickable 
                      v-close-popup 
                      @click="abrirTicketsMassa"
                    >
                      <q-item-section avatar>
                        <q-icon name="mdi-play" color="positive" />
                      </q-item-section>
                      <q-item-section>
                        <q-item-label>{{ $t('atendimentoMixinAtualizar.titles.iniciar') }}</q-item-label>
                      </q-item-section>
                    </q-item>
                    ...
                    <q-item 
                      clickable 
                      v-close-popup 
                      @click="retornarFilaTicketsMassa"
                    >
                      <q-item-section avatar>
                        <q-icon name="mdi-pause" color="warning" />
                      </q-item-section>
                      <q-item-section>
                        <q-item-label>{{ $t('atendimentoMixinAtualizar.titles.retornarFila') }}</q-item-label>
                      </q-item-section>
                    </q-item>
                    ...
                    <q-item 
                      clickable 
                      v-close-popup 
                      @click="resolverTicketsMassa"
                    >
                      <q-item-section avatar>
                        <q-icon name="mdi-check-circle" color="positive" />
                      </q-item-section>
                      <q-item-section>
                        <q-item-label>{{ $t('atendimentoInfoCabecalho.resolve') }}</q-item-label>
                      </q-item-section>
                    </q-item>
                    ...
                    <q-item 
                      clickable 
                      v-close-popup 
                      @click="fecharTicketsMassa"
                    >
                      <q-item-section avatar>
                        <q-icon name="mdi-close-circle" color="negative" />
                      </q-item-section>
                      <q-item-section>
                        <q-item-label>{{ $t('atendimentoMixinAtualizar.titles.encerrarAtendimento') }}</q-item-label>
                      </q-item-section>
                    </q-item>
                  </q-list>
                </q-btn-dropdown>
                ...
                <q-btn 
                  ...
                  dense
                  icon="mdi-close"
                  @click="cancelarSelecaoMassa"
                  class="q-ml-sm"
                >
                  <q-tooltip>{{ $t('common.cancel') || 'Cancelar' }}</q-tooltip>
                </q-btn>
              </div>
            </div>
          </div>
        ...
        <q-toolbar class="items-center">
          ...
          <div class="full-width row items-center q-gutter-xs">
            <q-btn 
              v-if="!modoSelecaoMassa"
              flat 
              dense 
              round
              icon="mdi-checkbox-multiple-marked" 
              @click="ativarModoSelecaoMassa"
              class="q-mr-sm"
              :color="$q.dark.isActive ? 'white' : 'primary'"
            >
              <q-tooltip>{{ $t('generalSupport.selectMultiple') || 'Selecionar múltiplos' }}</q-tooltip>
            </q-btn>
            <q-btn 
              v-else
              flat 
              ...
              color="negative"
            >
              <q-tooltip>{{ $t('common.cancel') || 'Cancelar seleção' }}</q-tooltip>
            </q-btn>
            <q-tabs
              v-model="tabTickets"
              narrow-indicator
            ...
            > {{ privateMessages?.length }}</q-badge>
              <q-tooltip> {{ $t('generalSupport.privateChats') }}  </q-tooltip>
                ...
                <q-item v-if="usuarioDados?.restrictedUser !== 'enabled'" clickable v-close-popup @click="showModalMensagem = true">
                  <q-item-section avatar>
                    <q-icon name="img:whatsapp-logo.png" />
                  </q-item-section>
                  <q-tooltip anchor="top middle">WhatsApp QR</q-tooltip>
                </q-item>

                <q-item v-if="usuarioDados?.restrictedUser !== 'enabled'" clickable v-close-popup @click="abrirModalMensagemInlineApi">
                  <q-item-section avatar>
                    <q-icon name="img:whatsapp-logo.png" />
                  </q-item-section>
                  <q-tooltip anchor="top middle">API Oficial</q-tooltip>
                </q-item>
            <q-tab
            ...
            <q-menu content-class="shadow-10" square>
              <div class="row q-pa-sm" style="min-width: 350px; max-width: 350px;">
                <div class="q-ma-sm">
                  ...
                  />
                  <q-toggle
                    v-if="(profile === 'admin' || (supervisorAdmin === 'disabled' && profile === 'super')) && pesquisaTickets.showAll"
                    class="q-ml-lg q-mt-md"
                    v-model="pesquisaTickets.includeClosed"
                    :label="$t('generalSupport.includeClosedTickets') || 'Incluir tickets fechados'"
                    @update:model-value="debounce(BuscarTicketFiltro(), 700)"
                  />
        ...
        <q-toolbar v-show="toolbarSearch" class="row q-gutter-sm q-py-sm items-center">
          ...
          <q-btn flat class="bg-padrao btn-rounded col-3" icon="mdi-comment-search-outline" @click="abrirModal" style="flex: 0.05; min-width: 50px;">
            <q-tooltip> {{ $t('generalSupport.searchMessage') }} </q-tooltip>
          </q-btn>
          <q-separator class="absolute-bottom" />
        </q-toolbar>
        ...
        >
          ...
          <ItemTicket
            ...
            :ticket="ticket"
            :filas="filas"
            :modoSelecao="modoSelecaoMassa"
            :isSelecionado="ticketsSelecionados.has(ticket.id)"
            @toggle-selecao="toggleSelecaoTicket"
          />
      ...
      <q-drawer v-if="!cRouteContatos && ticketFocado.id" v-model="drawerContact" show-if-above bordered side="right" content-class="bg-grey-1" @on-show="carregarIntegracaoStatus">
        ...
        <q-scroll-area style="height: calc(100vh - 70px)">
          <div class="q-pa-sm bg-grey-1 contact-panel">
            ...
            <q-card class="bg-white btn-rounded q-mt-sm" style="width: 100%; max-width: 100%;" bordered flat>
              <q-card-section class="text-center" style="max-width: 100%;">
                <q-avatar 
                  ...
                  <q-icon name="mdi-account" style="width: 100px; height: 100px" size="6em" color="grey-5" v-if="!ticketFocado.contact.profilePicUrl || ticketFocado.contact.profilePicUrl === 'null' || ticketFocado.contact.profilePicUrl === 'undefined'" />
                  <q-img 
                    v-if="ticketFocado.contact.profilePicUrl && ticketFocado.contact.profilePicUrl !== 'null' && ticketFocado.contact.profilePicUrl !== 'undefined'"
                ...
                >
                  <q-list style="min-width: 150px">
                    ...
                    <q-item
                      clickable
                      v-if="ticketFocado.whatsapp.wavoipToken && !bloquearWavoip && desabilitarInputWebchat"
                      @click="outcomingCallNovo(ticketFocado)"
                    >
                      <q-item-section>
                        {{ $t('generalSupport.contactDetails.call') }}
                      </q-item-section>
            ...
            <q-card class="bg-white btn-rounded q-mt-sm" style="width: 100%" bordered flat v-if="ticketFocado.channel !== 'telegram' && desabilitarInputWebchat">
              <q-card-section class="q-pa-none">
                <q-item tag="label" v-ripple>
                  <q-item-section>
            ...
            <q-card class="bg-white q-mt-sm btn-rounded" style="width: 100%" bordered flat :key="`ollama-${ticketFocado.id}`" v-if="ollamaAtivo === 'enabled' && ticketFocado.channel !== 'telegram' && !ticketFocado.isGroup && desabilitarInputWebchat">
              <q-card-section class="q-pa-none">
                <q-item tag="label" v-ripple>
                  <q-item-section>
                    <q-item-label>Ollama </q-item-label>
            ...
            <q-card class="bg-white q-mt-sm btn-rounded" style="width: 100%" bordered flat :key="`grok-${ticketFocado.id}`" v-if="grokAtivo === 'enabled' && ticketFocado.channel !== 'telegram' && !ticketFocado.isGroup && desabilitarInputWebchat">
              <q-card-section class="q-pa-none">
                <q-item tag="label" v-ripple>
                  <q-item-section>
                    <q-item-label>Grok </q-item-label>
            ...
            <q-card class="bg-white q-mt-sm btn-rounded" style="width: 100%" bordered flat :key="`gemini-${ticketFocado.id}`" v-if="geminiAtivo === 'enabled' && ticketFocado.channel !== 'telegram' && !ticketFocado.isGroup && desabilitarInputWebchat">
              <q-card-section class="q-pa-none">
                <q-item tag="label" v-ripple>
                  <q-item-section>
                    <q-item-label>Gemini </q-item-label>
            ...
            <q-card class="bg-white q-mt-sm btn-rounded" style="width: 100%" bordered flat :key="`qwen-${ticketFocado.id}`" v-if="qwenAtivo === 'enabled' && ticketFocado.channel !== 'telegram' && !ticketFocado.isGroup && desabilitarInputWebchat">
              <q-card-section class="q-pa-none">
                <q-item tag="label" v-ripple>
                  <q-item-section>
                    <q-item-label>Qwen </q-item-label>
            ...
            <q-card class="bg-white q-mt-sm btn-rounded" style="width: 100%" bordered flat :key="`claude-${ticketFocado.id}`" v-if="claudeAtivo === 'enabled' && ticketFocado.channel !== 'telegram' && !ticketFocado.isGroup && desabilitarInputWebchat">
              <q-card-section class="q-pa-none">
                <q-item tag="label" v-ripple>
                  <q-item-section>
                    <q-item-label>Claude </q-item-label>
            ...
            <q-card class="bg-white q-mt-sm btn-rounded" style="width: 100%" bordered flat :key="`deepseek-${ticketFocado.id}`" v-if="deepseekAtivo === 'enabled' && ticketFocado.channel !== 'telegram' && !ticketFocado.isGroup && desabilitarInputWebchat">
              <q-card-section class="q-pa-none">
                <q-item tag="label" v-ripple>
                  <q-item-section>
                    <q-item-label>Deepseek </q-item-label>
            ...
            <q-card class="bg-white btn-rounded q-mt-sm" style="width: 100%" bordered flat v-if="desabilitarInputWebchat">
              ...
              <q-card-section class="q-pa-sm">
                <q-item>
                  <q-card-section class="text-bold q-pa-sm">
                    <q-btn 
                      v-if="(ticketFocado.channel === 'baileys' || ticketFocado.channel === 'uazapi' || ticketFocado.channel === 'meow' || ticketFocado.channel === 'whatsapp')&& !ticketFocado.isGroup"
                      flat 
                      class="btn-outline btn-small" 
      ...
      <!-- Modal: Selecionar Template (API Oficial - Inline) -->
      <q-dialog v-model="modalVisivelTemplateInlineApi" position="standard" @hide="modalVisivelTemplateInlineApi = false">
        <q-card style="max-width: 900px; width: 100%">
          <q-card-section class="row items-center q-pb-none">
            <div class="text-h6">Escolha um template</div>
            <q-space />
          ...
          <q-card-section class="q-pt-md">
            <div class="row q-col-gutter-lg">
              <div class="col-12 col-md-6">
                <q-select
                  v-model="selectedTemplateInlineApi"
                  :options="templatesInlineApi.map(t => ({ label: `${t.name} (${t.language || ''})`, value: t }))"
                  ...
                  emit-value
                  map-options
                  @update:model-value="handleTemplateSelectionInlineApi"
                />

                <div class="col-12 q-mt-md" v-if="selectedTemplateComponentsInlineApi.length">
                  <div v-for="(input, idx) in selectedTemplateComponentsInlineApi" :key="`inline-comp-${idx}`">
                    <q-input v-model="input.value" :label="input.label" filled outlined dense class="q-mb-sm" @input="atualizarPreviewInlineApi" />
                  </div>
                </div>
              </div>

              <div class="col-12 col-md-6">
                <div class="text-subtitle1 text-weight-medium q-mb-sm">Prévia do template</div>
                <div class="phone-mockup">
                  <div class="phone-frame">
                    <div class="phone-notch"></div>
                    <div class="phone-screen mass-template-preview-screen">
                      <div class="mass-template-preview-bubble">
                        <div v-if="previewHeaderContentInlineApi" class="mass-preview-header" v-html="previewHeaderContentInlineApi"></div>
                        <div v-if="previewBodyContentInlineApi" class="mass-preview-body" v-html="previewBodyContentInlineApi"></div>
                        <div v-if="previewFooterContentInlineApi" class="mass-preview-footer" v-html="previewFooterContentInlineApi"></div>
                        <div v-if="previewButtonsInlineApi && previewButtonsInlineApi.length" class="mass-preview-buttons q-mt-sm">
                          <q-btn v-for="(btn, index) in previewButtonsInlineApi" :key="index" unelevated class="q-mb-xs full-width preview-button" size="sm" :label="btn.text || btn.title || btn.name" color="blue-grey-1" text-color="primary" />
                        </div>
                      </div>
                    </div>
                  </div>
          ...
          <q-card-actions align="right">
            <q-btn flat label="Cancelar" @click="modalVisivelTemplateInlineApi = false" />
            <q-btn :label="$t('common.send')" color="primary" :disabled="!selectedTemplateInlineApi || sendingInlineApi" :loading="sendingInlineApi" @click="enviarTemplateSelecionadoInlineApi" />
          </q-card-actions>
        </q-card>
      </q-dialog>

      <!-- Modal: Iniciar Conversa Avulsa (API Oficial) -->
      <q-dialog v-model="showModalMensagemInlineApi" position="standard" @hide="showModalMensagemInlineApi = false">
        <q-card style="max-width: 900px; width: 100%">
          <q-card-section class="row items-center q-pb-none">
            <div class="text-h6">Iniciar conversa (API Oficial)</div>
            <q-space />
            <q-btn flat round dense icon="close" v-close-popup @click="showModalMensagemInlineApi = false" />
          </q-card-section>
          ...
          <q-card-section class="q-pt-md">
            <div class="row q-col-gutter-lg">
              <div class="col-12 col-md-6">
                ...
                >
                  <template v-slot:append>
                    <q-btn dense flat round icon="mdi-content-paste" @click="pasteIntoNumeroInlineApi" aria-label="colar-telefone-inline-api" />
                  </template>
                </c-input>

                <div class="row q-mt-sm">
                  <q-btn dense unelevated color="primary" @click="buscarTemplateWabaInlineApi" icon="email" label="Selecionar Template" />
                </div>

                <div class="col q-mt-md" v-if="selectedTemplateInlineApi">
                  <div v-for="(input, idx) in selectedTemplateComponentsInlineApi" :key="`inline-api-input-modal-${idx}`" class="q-mb-sm">
                    <q-input dense outlined v-model="input.value" :label="input.label" @input="atualizarPreviewInlineApi" />
                  </div>
                </div>
              </div>
          ...
          <q-card-actions align="right">
            <q-btn flat label="Cancelar" @click="showModalMensagemInlineApi = false" />
            <q-btn :label="$t('common.send')" color="primary" :disabled="!selectedTemplateInlineApi || sendingInlineApi" :loading="sendingInlineApi" @click="enviarTemplateSelecionadoInlineApiFromModal" />
          </q-card-actions>
        </q-card>
      </q-dialog>
      ...
      <q-dialog v-model="showImageModal">
        <q-card v-if="usuarioDados?.restrictedUser !== 'enabled'">
          <q-card-section>
            ...
            <q-icon v-else name="mdi-account" style="width: 400px; height: 400px" size="6em" color="grey-5" />
          </q-card-section>
          <q-card-actions align="right">
              ...
              <q-list>
                <q-item v-for="oportunidade in oportunidadesContato" :key="oportunidade.id">
                  <q-item-section>
                    ...
                    <q-item-label caption>
                      {{ $t('generalSupport.status') }}: {{ getStatusLabel(oportunidade.statusOriginal || oportunidade.status) }}
                    </q-item-label>
                    <q-item-label caption>
    ...
    <!-- Dialog para seleção de Wavoip (Novo) -->
    <q-dialog v-model="showSelectWavoipDialogNovo">
      <q-card style="min-width: 320px; max-width: 90vw;">
        <q-card-section>
          <div class="text-h6">{{ $t('generalSupport.selectWavoip') }} (Novo)</div>
        </q-card-section>
        <q-card-section>
          <q-list bordered separator>
            <q-item v-for="opt in wavoipOptionsNovo" :key="opt.value" clickable @click="selectedWavoipTokenNovo = opt.value" :active="selectedWavoipTokenNovo === opt.value">
              <q-item-section avatar>
                <q-icon :name="opt.icon" />
              </q-item-section>
              <q-item-section>{{ opt.label }}</q-item-section>
              <q-item-section side>
                <q-radio v-model="selectedWavoipTokenNovo" :val="opt.value" />
              </q-item-section>
            </q-item>
          </q-list>
        </q-card-section>
        <q-card-actions align="right">
          <q-btn flat :label="$t('common.cancel')" color="negative" @click="cancelarSelecaoWavoipNovo" />
          <q-btn flat :label="$t('webphone.tooltips.call')" color="primary" :disable="!selectedWavoipTokenNovo" @click="confirmarSelecaoWavoipNovo" />
        </q-card-actions>
      </q-card>
    </q-dialog>

    <!-- Modal de Transferência em Massa -->
    <q-dialog v-model="modalTransferirTicketMassa"
      @hide="modalTransferirTicketMassa = false"
      persistent>
      <q-card class="q-pa-md"
        ...
        <q-card-section>
          ...
          <div class="text-caption q-mt-sm">
            {{ $t('generalSupport.transferSelectedTickets') || 'Transferir tickets selecionados' }} ({{ ticketsSelecionados.size }})
          </div>
        </q-card-section>
        <q-card-section>
          <q-select square
            outlined
            v-model="filaSelecionadaMassa"
            :options="filas"
            emit-value
            map-options
            option-value="id"
        ...
        <q-card-section>
          <q-select square
            outlined
            v-model="usuarioSelecionadoMassa"
            :options="usuarios.filter(filterUsersMassa)"
            emit-value
            map-options
            option-value="id"
            option-label="name"
        ...
        <q-card-actions align="right">
          ...
          <q-btn
            :label="$t('common.save')"
            color="primary"
            @click="confirmarTransferenciaMassa" />
        </q-card-actions>
      </q-card>
    </q-dialog>
...
import { EnviarTextoWaba, BuscarTemplates, EnviarTemplateComponenteWaba } from 'src/service/waba.js'
import { EnviarTextoInstagram } from 'src/service/instagram.js'
...
export default {
  ...
  data() {
    return {
      ...
      modoSelecaoMassa: false,
      ticketsSelecionados: new Set(),
      modalTransferirTicketMassa: false,
      filaSelecionadaMassa: null,
      usuarioSelecionadoMassa: null,
      assistentes: [],
      numerosDeTelefone: [],
      ...
      ticketParaChamada: null,
      showSelectWavoipDialogNovo: false,
      wavoipOptionsNovo: [],
      selectedWavoipTokenNovo: null,
      ticketParaChamadaNovo: null,
      loadingLidUpdate: false,
      modalConvites: false,
      convitesUsuario: [],
      loadingConvites: false,
      // Inline API Oficial template flow
      whatsappIdInlineApi: null,
      numeroInlineApi: '',
      modalVisivelTemplateInlineApi: false,
      templatesInlineApi: [],
      selectedTemplateInlineApi: null,
      selectedTemplateComponentsInlineApi: [],
      previewHeaderContentInlineApi: '',
      previewBodyContentInlineApi: '',
      previewFooterContentInlineApi: '',
      previewButtonsInlineApi: [],
      loadingTemplatesInlineApi: false,
      sendingInlineApi: false,
      sentCountInlineApi: 0,
      // QR / quick-send (non-API inline) flow
      whatsappIdInline: null,
      numeroInline: '',
      mensagemInline: '',
      loadingInline: false,
      // Modal for official API quick-start (avulsa)
      showModalMensagemInlineApi: false,
    }
  },
  watch: {
    ...
    ticketFocado: {
      handler(newVal, oldVal) {
        ...
        try{
          ...
          console.error('Erro ao enviar paused indicator:', e)
        }
      },
      immediate: true
    },
  ...
  methods: {
    ...
    filtrarMensagensRapidasDoUsuario(mensagens) {
      ...
      return mensagens
        ...
        .filter(mensagem => {
          // Excluir mensagens com user null
          // if (mensagem?.user === null || mensagem?.user === undefined) {
          //   return false
          // }
          return this.isMensagemRapidaDoUsuario(mensagem)
        })
    },
    ...
    // --- Inline API Oficial: template modal/preview/send helpers ---
    resolveWhatsappSelectionInlineApi() {
      const sel = this.whatsappIdInlineApi;
      if (!sel) return null;
      // sel agora é o ID (string/number)
      const found = this.cSessions.find(w => w.id === sel);
      if (found && found.tokenAPI && (found.phone_number_id || found.phoneNumberId || found.wabaId)) {
        return {
          id: found.id,
          tokenAPI: found.tokenAPI,
          phone_number_id: found.phone_number_id || found.phoneNumberId || found.wabaId
        };
      }
      return null;
    },
    async buscarTemplateWabaInlineApi(openModal = true) {
      const sel = this.resolveWhatsappSelectionInlineApi();
      if (!sel) {
        this.$q.notify({ type: 'negative', message: 'Selecione um canal válido' });
        return;
      }
      if (!sel.tokenAPI) {
        this.$q.notify({ type: 'negative', message: 'Canal sem token da API' });
        return;
      }
      if (!sel.phone_number_id) {
        this.$q.notify({ type: 'negative', message: 'Canal sem phone_number_id' });
        return;
      }
      this.loadingTemplatesInlineApi = true;
      try {
        const response = await BuscarTemplates(sel.tokenAPI);
        const data = response?.data?.data ?? response?.data ?? [];
        this.templatesInlineApi = Array.isArray(data)
          ? data.filter(t => !t.status || String(t.status).toUpperCase() === 'APPROVED')
          : [];
        if (!this.templatesInlineApi.length) {
          this.$q.notify({ type: 'warning', message: 'Nenhum template aprovado encontrado' });
          return;
        }
        if (openModal) {
          this.modalVisivelTemplateInlineApi = true;
        }
      } catch (error) {
        ...
        this.$q.notify({
          type: 'negative',
          message: detail
            ? `Erro ao buscar templates: ${detail}`
            : 'Erro ao buscar templates'
        });
      } finally {
        this.loadingTemplatesInlineApi = false;
      }
    },
    handleTemplateSelectionInlineApi(templateWrapper) {
      // Garante que selectedTemplateInlineApi seja sempre o template correto
      const tpl = templateWrapper?.value ?? templateWrapper;
      this.selectedTemplateInlineApi = tpl;
      this.selectedTemplateComponentsInlineApi = [];
      if (!tpl || !tpl.components) return;
      tpl.components.forEach(component => {
        if (component.type === 'HEADER') {
          if (["VIDEO", "IMAGE", "DOCUMENT"].includes(component.format)) {
            this.selectedTemplateComponentsInlineApi.push({
      ...
      this.atualizarPreviewInlineApi();
    },
    atualizarPreviewInlineApi() {
      const tpl = this.selectedTemplateInlineApi;
      if (!tpl) {
        this.previewBodyContentInlineApi = '';
        this.previewHeaderContentInlineApi = '';
        this.previewFooterContentInlineApi = '';
        this.previewButtonsInlineApi = [];
        return;
      }
      console.log('atualizarPreviewInlineApi components', this.selectedTemplateComponentsInlineApi);
      const header = tpl.components?.find(c => c.type === 'HEADER');
      if (header) {
        if (header.format === 'TEXT') {
          this.previewHeaderContentInlineApi = header.text || '';
        } else if (header.format === 'IMAGE' || header.format === 'VIDEO' || header.format === 'DOCUMENT') {
          const headerComponent = this.selectedTemplateComponentsInlineApi.find(
            c => c.key === `header_${header.format.toLowerCase()}`
          );
          const headerUrl = headerComponent?.value || '';
          console.log('atualizarPreviewInlineApi headerUrl ->', headerUrl);
          if (headerUrl) {
            if (header.format === 'IMAGE') {
              // Use an <img> tag and a background fallback container to improve preview reliability
              this.previewHeaderContentInlineApi = `
                <div style="width:100%; border-radius:8px 8px 0 0; overflow:hidden;">
                  <img src="${headerUrl}" alt="Template Image" style="width:100%; display:block;" onerror="this.style.display='none'" />
                  <div style="background-image:url('${headerUrl}'); background-size:cover; background-position:center; width:100%; height:200px;"></div>
                </div>
              `;
            } else if (header.format === 'VIDEO') {
              this.previewHeaderContentInlineApi = `<video controls style="width: 100%; border-radius: 8px 8px 0 0; display: block;"><source src="${headerUrl}" type="video/mp4" /></video>`;
            } else if (header.format === 'DOCUMENT') {
              this.previewHeaderContentInlineApi = `<div style="padding: 8px; background: #f0f0f0; border-radius: 6px; margin-bottom: 8px;"><i class="material-icons" style="vertical-align: middle;">description</i> Documento</div>`;
            }
          } else {
            // Placeholder quando não há URL
            if (header.format === 'IMAGE') {
              this.previewHeaderContentInlineApi = `<div style="padding: 40px; background: #e0e0e0; text-align: center; border-radius: 8px 8px 0 0; color: #757575;">Imagem do template</div>`;
            } else if (header.format === 'VIDEO') {
              this.previewHeaderContentInlineApi = `<div style="padding: 40px; background: #e0e0e0; text-align: center; border-radius: 8px 8px 0 0; color: #757575;">Vídeo do template</div>`;
            } else if (header.format === 'DOCUMENT') {
              this.previewHeaderContentInlineApi = `<div style="padding: 20px; background: #e0e0e0; text-align: center; border-radius: 8px 8px 0 0; color: #757575;">Documento do template</div>`;
            }
          }
        }
      } else {
      ...
      this.previewBodyContentInlineApi = body?.text || '';
      const footer = tpl.components?.find(c => c.type === 'FOOTER');
      this.previewFooterContentInlineApi = footer?.text || '';
      const buttons = tpl.components?.find(c => c.type === 'BUTTONS');
      this.previewButtonsInlineApi = buttons?.buttons || [];
    },
    async enviarTemplateSelecionadoInlineApi() {
      if (!this.selectedTemplateInlineApi) {
        this.$q.notify({ type: 'warning', message: this.$t('massaTemplate.notifications.chooseTemplate') || 'Selecione um template' });
        return;
      }
      const sel = this.resolveWhatsappSelectionInlineApi();
      if (!sel) {
        this.$q.notify({ type: 'negative', message: this.$t('generalSupport.selectChannel') || 'Selecione o canal' });
        return;
      }
      const phone = String(this.numeroInlineApi || '').replace(/\D/g, '');
      if (!phone) {
        this.$q.notify({ type: 'negative', message: this.$t('generalSupport.invalidNumber') || 'Número inválido' });
        return;
      }
      // Resolver contato/ticket baseado no número selecionado (numeroInlineApi)
      let contato = null;
      let ticketExistente = null;
      let ticketIdToUse = null;
      try {
        const respContato = await ObterContatoPeloNumero(phone);
        contato = respContato?.data;
      } catch (err) {
        try {
          const novo = { name: this.numeroInlineApi, number: this.numeroInlineApi };
          const respCriar = await CriarContato(novo);
          contato = respCriar?.data;
        } catch (createErr) {
          console.error('Erro ao obter/criar contato para envio inline API', createErr);
          contato = null;
        }
      }
      ...
      if (contato && contato.id) {
        try {
          const resp = await VerificarTicketsAbertos(contato.id, sel.id);
          if (resp && resp.data && resp.data.id) ticketExistente = resp.data;
        } catch (err) {
          if (err && err.status === 409 && err.data) {
            if (err.data.ticket) ticketExistente = err.data.ticket;
            else if (err.data.error) {
              try { ticketExistente = JSON.parse(err.data.error); } catch (e) { ticketExistente = null; }
            }
          }
        }
      }
      ...
      if (ticketExistente && ticketExistente.id) {
        ticketIdToUse = ticketExistente.id;
      } else if (contato && contato.id && sel && sel.id && this.usuario && this.usuario.userId) {
        try {
          const ticketData = {
          ...
          const { data: createdTicket } = await CriarTicket(ticketData);
          if (createdTicket && createdTicket.id) ticketIdToUse = createdTicket.id;
        } catch (err) {
          console.error('Erro ao criar ticket para envio inline API', err);
        }
      }
      this.sendingInlineApi = true;
      try {
        const payload = {
          from: phone,
          phone_number_id: sel.phone_number_id,
          ticketId: ticketIdToUse,
          idFront: uid(),
          language: this.selectedTemplateInlineApi.language,
          templateName: this.selectedTemplateInlineApi.name,
          components: [],
          body: JSON.stringify(this.selectedTemplateInlineApi.components),
          tokenApi: sel.tokenAPI,
          mediaType: 'templates',
          sendType: 'templates',
        };

        // HEADER: só se for mídia
        const header = this.selectedTemplateInlineApi.components?.find(c => c.type === 'HEADER');
        if (header && ["IMAGE", "VIDEO", "DOCUMENT"].includes(header.format)) {
          const headerInput = this.selectedTemplateComponentsInlineApi.find(
            c => c.key === `header_${header.format.toLowerCase()}`
          );
          const headerUrl = headerInput?.value || '';

          // Se header existe no template, mas usuário não preencheu URL, bloquear envio
          if (!headerUrl) {
            this.$q.notify({ type: 'negative', message: this.$t('massaTemplate.notifications.headerUrlRequired') || 'Preencha a URL do header antes de enviar' });
            this.sendingInlineApi = false;
            return;
          }

          // Adicionar componente HEADER com value + link e setar mediaUrl para o payload
          ...
          if (header.format === 'IMAGE') {
            payload.mediaUrl = headerUrl;
          }
        }

        // BODY: só se houver variáveis
        const body = this.selectedTemplateInlineApi.components?.find(c => c.type === 'BODY');
        if (body) {
          ...
          if (variableMatches && variableMatches.length > 0) {
            ...
            variableMatches.forEach(variable => {
              const variableNumber = variable.replace(/{{|}}/g, '');
              const input = this.selectedTemplateComponentsInlineApi.find(
                c => c.key === `variable_${variableNumber}`
              );
              variables[variableNumber - 1] = input?.value || '';
            });
            payload.components.push({ type: 'BODY', variables });
          }
        }

        // NÃO adicionar BUTTONS

        // Filtrar componentes inválidos (HEADER pode ter link ou value)
        payload.components = payload.components.filter(comp => {
          if (comp.type === 'HEADER') {
            return comp.format && (comp.value || comp.link);
          }
          if (comp.type === 'BODY') {
            return Array.isArray(comp.variables) && comp.variables.length > 0;
          }
          return false;
        });

        // Logs detalhados solicitados
        console.log('payload inline', JSON.stringify(payload, null, 2));
        console.log('ticketId', payload.ticketId);
        console.log('phone_number_id', payload.phone_number_id);
        console.log('tokenApi', payload.tokenApi);
        console.log('headerComponent', payload.components.find(c => c.type === 'HEADER'));

        const response = await EnviarTemplateComponenteWaba({ ...payload, dataJson: JSON.stringify(payload.components) });

        // Construir notificação semelhante ao envio normal
        let message = this.$t('generalSupportScript.messageSent') + phone;
        if (response && response.data && response.data.ticketId) {
        ...
        this.sentCountInlineApi++;
        // Após envio, focar/navegar para o ticket aberto correspondente
        if (this.ticketFocado && this.ticketFocado.id) {
          if (typeof this.abrirTicket === 'function') {
            this.abrirTicket(this.ticketFocado);
          } else if (this.$router && this.$router.push) {
            this.$router.push({ name: 'atendimento', params: { ticketId: this.ticketFocado.id } });
          } else if (this.setTicketFocado) {
            this.setTicketFocado(this.ticketFocado.id);
          }
          if (typeof this.scrollToBottomChat === 'function') {
            this.scrollToBottomChat();
          }
        }
        this.modalVisivelTemplateInlineApi = false;
        // Limpar seleção para facilitar um novo envio: limpar canal (inquilino) e template selecionado
        
        this.selectedTemplateInlineApi = null;
        this.selectedTemplateComponentsInlineApi = [];
        this.previewHeaderContentInlineApi = '';
        this.previewBodyContentInlineApi = '';
        this.previewFooterContentInlineApi = '';
      } catch (error) {
        console.error('Erro enviar template inline api', error);
        this.$q.notify({ type: 'negative', message: this.$t('massaTemplate.notifications.errorSendingTemplate') || 'Erro ao enviar template' });
      } finally {
        this.sendingInlineApi = false;
      }
    },
    pasteIntoNumeroInlineApi() {
      navigator.clipboard?.readText().then(text => {
        if (text) this.numeroInlineApi = text;
      }).catch(() => {});
    },
    pasteIntoNumeroInline() {
      navigator.clipboard?.readText().then(text => {
        if (text) this.numeroInline = text;
      }).catch(() => {});
    },

    async enviarMensagemInline() {
      if (!this.whatsappIdInline) {
        this.$q.notify({ color: 'negative', position: 'top', message: this.$t('mainLayout.novo.whatsappNotFound') });
        return;
      }

      if (!this.numeroInline) {
        this.$q.notify({ color: 'negative', position: 'top', message: this.$t('mainLayout.novo.numberNotFound') });
        return;
      }

      // Prefill the main modal composer and open it for confirmation/send
      const session = this.whatsapps && this.whatsapps.find(w => w.id === this.whatsappIdInline);
      if (session) {
        this.whatsappId = { value: session.id, type: session.type };
      } else {
        this.whatsappId = { value: this.whatsappIdInline, type: 'waba' };
      }

      this.numero = this.numeroInline;
      this.mensagem = this.mensagemInline;
      this.showModalMensagem = true;
    },

    abrirModalMensagemInlineApi() {
      // prefills: prefer explicit inline fields, then QR quick fields, then main composer
      const defaultSession = (this.cSessionsOptions && this.cSessionsOptions.length) ? this.cSessionsOptions[0].value : null;
      this.whatsappIdInlineApi = this.whatsappIdInlineApi || this.whatsappIdInline || (this.whatsappId && this.whatsappId.value) || defaultSession;
      this.numeroInlineApi = this.numeroInlineApi || this.numeroInline || this.numero || '';
      // Ensure templates are loaded if needed
      if (!this.templatesInlineApi || this.templatesInlineApi.length === 0) {
        this.buscarTemplateWabaInlineApi && this.buscarTemplateWabaInlineApi(false);
      }
      this.showModalMensagemInlineApi = true;
    },
    async enviarTemplateSelecionadoInlineApiFromModal() {
      try {
        await this.enviarTemplateSelecionadoInlineApi();
        this.showModalMensagemInlineApi = false;
      } catch (e) {
        console.error('Erro enviar template from modal', e);
      }
    },
    async fazerChamada() {
      try {
    ...
    async editarOportunidade(oportunidade) {
      // Garante que o status seja o value original, não o label
      this.oportunidadeSelecionada = {
        ...oportunidade,
        status: oportunidade.statusOriginal || oportunidade.status
      }
    ...
    async enviarMensagemNormal() {
      ...
      let message = this.$t('generalSupportScript.messageSent') + this.numero;
      ...
      if (ticketExistente && ticketExistente.id) {
        ...
        if (ticket && ticket.id) {
          ...
          this.$q.notify({
            color: 'negative',
            position: 'top',
            message: 'Erro: Ticket criado mas sem ID válido.',
          });
        }
      }
    },
    ...
    async updateSelectedFilter() {
      ...
      // Removido: await this.consultarTickets()
      // A consulta será feita por BuscarTicketFiltro() que é chamado após esta função
    ...
    async sendProtocol (ticketFocado) {
      this.$q.dialog({
        ...
        if (data) {
          ...
          else if(ticketFocado.channel === 'instagram'){
            await EnviarTextoInstagram(messageInstagram)
          } 
          else if(ticketFocado.channel === 'messenger'){
            await EnviarTextoMessenger(messageMessenger)
          }
    ...
    startPausedIndicator(state = 'paused', oldVal) {
      ...
      oldVal.whatsapp.type === 'meow')
        ...
        SendPausedIndicator(oldVal.id, {state, ticket: oldVal})
          ...
          .catch(error => {
            console.error('Erro ao enviar paused indicator 1:', error);
          });
      }
    },
    ...
    outcomingCallNovo(ticketFocado) {
      // Filtra todos os whatsapps conectados com wavoipToken válido
      const whatsappsDisponiveis = this.whatsapps.filter(w => w.wavoipToken && w.status === 'CONNECTED');
      if (whatsappsDisponiveis.length <= 1) {
        // Só existe um, usa direto
        const token = whatsappsDisponiveis[0]?.wavoipToken || ticketFocado.whatsapp.wavoipToken;
        this.$store.dispatch('webphone_novo/outcomingCall', {
          phone: ticketFocado.contact.number,
          contact_name: ticketFocado.contact.name,
          chat_id: ticketFocado.id,
          token: token,
        ...
        // Mais de um, abre o modal de seleção
        this.wavoipOptionsNovo = whatsappsDisponiveis.map(w => ({
          label: w.name + (w.id ? ` (#${w.id})` : ''),
          value: w.wavoipToken,
          icon: w.type ? `img:${w.type}-logo.png` : 'mdi-whatsapp',
          id: w.id
        }));
        this.selectedWavoipTokenNovo = null;
        this.ticketParaChamadaNovo = ticketFocado; // Salva o ticket para usar depois
        this.showSelectWavoipDialogNovo = true;
      }
    },
    ...
    confirmarSelecaoWavoipNovo() {
      if (!this.selectedWavoipTokenNovo || !this.ticketParaChamadaNovo) return;
      this.$store.dispatch('webphone_novo/outcomingCall', {
        phone: this.ticketParaChamadaNovo.contact.number,
        contact_name: this.ticketParaChamadaNovo.contact.name,
        chat_id: this.ticketParaChamadaNovo.id,
        token: this.selectedWavoipTokenNovo,
        profile_picture: this.ticketParaChamadaNovo.contact.profilePicUrl || '',
        instances: [this.selectedWavoipTokenNovo]
      });
      this.showSelectWavoipDialogNovo = false;
      this.ticketParaChamadaNovo = null;
      this.selectedWavoipTokenNovo = null;
    },
    cancelarSelecaoWavoipNovo() {
      this.showSelectWavoipDialogNovo = false;
      this.ticketParaChamadaNovo = null;
      this.selectedWavoipTokenNovo = null;
    },
    ...
    // Métodos de seleção em massa
    ativarModoSelecaoMassa() {
      this.modoSelecaoMassa = true;
      this.ticketsSelecionados.clear();
    },
    cancelarSelecaoMassa() {
      this.modoSelecaoMassa = false;
      this.ticketsSelecionados.clear();
    },
    toggleSelecaoTicket(ticketId, selecionado) {
      if (selecionado) {
        this.ticketsSelecionados.add(ticketId);
      } else {
        this.ticketsSelecionados.delete(ticketId);
      }
      // Se não houver mais tickets selecionados, desativar modo seleção
      if (this.ticketsSelecionados.size === 0) {
        this.modoSelecaoMassa = false;
      }
    },
    obterTicketsSelecionados() {
      const todosTickets = [
        ...this.openTicketsReversed,
        ...this.pendingTicketsReversed,
        ...this.pendingTicketsChatbotReversed,
        ...this.closedTicketsReversed,
        ...this.openTicketsGroupsReversed,
        ...this.pendingTicketsGroupsReversed,
        ...this.closedTicketsGroupsReversed
      ];
      return todosTickets.filter(ticket => this.ticketsSelecionados.has(ticket.id));
    },
    async abrirTicketsMassa() {
      const tickets = this.obterTicketsSelecionados();
      ...
      for (const ticket of tickets) {
        if (ticket.status === 'pending') {
          try {
            ...
            console.error(`Erro ao abrir ticket ${ticket.id}:`, error);
            erros++;
          }
        }
      }
      ...
      this.loading = false;
      this.$q.notify({
        type: sucesso > 0 ? 'positive' : 'negative',
        message: `${sucesso} ticket(s) aberto(s)${erros > 0 ? `. ${erros} erro(s).` : ''}`,
        progress: true
      });
      
      this.cancelarSelecaoMassa();
    },
    async retornarFilaTicketsMassa() {
      const tickets = this.obterTicketsSelecionados();
      if (tickets.length === 0) return;
      ...
      for (const ticket of tickets) {
        if (ticket.status === 'open') {
          try {
            await AtualizarStatusTicket(ticket.id, 'pending', null);
            sucesso++;
          } catch (error) {
            console.error(`Erro ao retornar ticket ${ticket.id}:`, error);
            erros++;
          }
        }
      }
      ...
      this.$q.notify({
        type: sucesso > 0 ? 'positive' : 'negative',
        message: `${sucesso} ticket(s) retornado(s) para a fila${erros > 0 ? `. ${erros} erro(s).` : ''}`,
        progress: true
      });
      
      this.cancelarSelecaoMassa();
    },
    async fecharTicketsMassa() {
      const tickets = this.obterTicketsSelecionados();
      if (tickets.length === 0) return;
      
      this.$q.dialog({
        title: this.$t('atendimentoMixinAtualizar.titles.encerrarAtendimento'),
        message: `${this.$t('atendimentoMixinAtualizar.dialogs.ticketEncerrar', { ticketId: tickets.length })} ${tickets.length} ticket(s)?`,
        cancel: {
          label: this.$t('common.no'),
          color: 'negative',
          push: true,
        ...
        for (const ticket of tickets) {
          try {
            await AtualizarStatusTicketForcado(ticket.id, 'closed', userId);
            sucesso++;
          } catch (error) {
        ...
        this.$q.notify({
          type: sucesso > 0 ? 'positive' : 'negative',
          message: `${sucesso} ticket(s) fechado(s)${erros > 0 ? `. ${erros} erro(s).` : ''}`,
          progress: true
        });
    ...
    async resolverTicketsMassa() {
      ...
      for (const ticket of tickets) {
        if (ticket.status !== 'closed') {
          try {
            await AtualizarStatusTicket(ticket.id, 'closed', userId);
            sucesso++;
          } catch (error) {
            console.error(`Erro ao resolver ticket ${ticket.id}:`, error);
            erros++;
          }
        }
      }
      ...
      this.$q.notify({
        type: sucesso > 0 ? 'positive' : 'negative',
        message: `${sucesso} ticket(s) resolvido(s)${erros > 0 ? `. ${erros} erro(s).` : ''}`,
        progress: true
      });
      
      this.cancelarSelecaoMassa();
    },
    abrirModalTransferirMassa() {
      this.modalTransferirTicketMassa = true;
    ...
    filterUsersMassa(element, index, array) {
      const fila = this.filaSelecionadaMassa
    ...
    async confirmarTransferenciaMassa() {
      if (!this.filaSelecionadaMassa) {
        this.$q.notify({
          type: 'warning',
          message: this.$t('atendimentoInfoCabecalho.selectDestinationQueue'),
          progress: true
        });
        return;
      }
      ...
      for (const ticket of tickets) {
        try {
          await AtualizarTicket(ticket.id, {
            userId: this.usuarioSelecionadoMassa,
            queueId: this.filaSelecionadaMassa,
            status: this.usuarioSelecionadoMassa == null ? 'pending' : 'open',
            isTransference: 1
          });
          sucesso++;
        } catch (error) {
          console.error(`Erro ao transferir ticket ${ticket.id}:`, error);
          erros++;
        }
      }
      ...
      this.$q.notify({
        type: sucesso > 0 ? 'positive' : 'negative',
        message: `${sucesso} ticket(s) transferido(s)${erros > 0 ? `. ${erros} erro(s).` : ''}`,
        progress: true
      });
      
      this.modalTransferirTicketMassa = false;
      this.filaSelecionadaMassa = null;
      this.usuarioSelecionadoMassa = null;
      this.cancelarSelecaoMassa();
    },
    async efetuarLogout() {
    ...
    async listarUsuarios() {
      try {
        ...
        // Filtrar e ordenar usuários alfabeticamente por nome
    ...
    async atualizarUsuario() {
      try {
        ...
        // Ordenar filas alfabeticamente por nome
        this.filasUser = (data.queues || []).sort((a, b) => {
          const nomeA = (a.queue || '').toLowerCase()
          const nomeB = (b.queue || '').toLowerCase()
    ...
    async carregarOportunidadesContato() {
      ...
      try {
        ...
        this.oportunidadesContato = response.data.data.map(oportunidade => ({
          ...oportunidade,
          statusOriginal: oportunidade.status
        }))
  ...
  async mounted() {
    ...
    try {
      ...
      if (error?.code !== 'ECONNABORTED' && !error?.message?.includes('timeout')) {
        console.error('Erro ao listar tenant:', error)
      }
    }
    try {
    ...
    try {
      ...
      // Silenciar erros de timeout - não mostrar no console
      if (error?.code !== 'ECONNABORTED' && !error?.message?.includes('timeout')) {
        console.error('Erro ao listar mensagens rápidas:', error)
      }
    }
...
.bulk-actions-bar
  ...
  // Efeito de profundidade usando overlay
  &::before
    content: ''
    position: absolute
    top: 0
...
/* Phone mockup for template preview */
.phone-mockup {
  display: flex;
  justify-content: center;
  align-items: center;
...
.mass-template-preview-bubble {
  background: transparent;
  padding: 6px;
}
.mass-preview-header img {
  max-width: 100%;
...
.mass-preview-buttons .preview-button {
  border-radius: 6px;
}
